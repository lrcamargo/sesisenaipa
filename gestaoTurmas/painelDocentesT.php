...
$feriasTurmas = []; // [codigoTurma][data] = descricao
...
$stmtApp = $pdo->prepare("
    SELECT t.codigo, t.diasSemana,
           tt.turno, tt.instrutor, COALESCE(l.nome,'') AS nomeLab
    FROM app_turmas t
...
/*
 * Verifica se uma turma está em período de férias numa data específica.
 * Retorna a descrição ou null.
 */
...
foreach(array_keys($appTurnas) as $c) if(!in_array($c,$todasTurmas)) $todasTurmas[] = $c;
...
function turmaDeveExibirNaTurno(string $cod, string $turno, array $diasSemana, array $diasBits): bool {
    global $vincIdx, $cacheJson, $appTurnas;

    // APP manual — exibe se tem esse turno com pelo menos 1 dia da semana
    if(isset($appTurnas[$cod][$turno])){
        $mask = $appTurnas[$cod][$turno]['diasSemana'];
    ...
    foreach($turnosConfig as $turnoKey => $tConf){
        ...
        foreach($diasSemana as $di => $data){
            ...
            if(!$info || empty($info['instrutor'])){
                ...
                if(temFerias($cod, $data)) continue; // férias → sem alerta
                if(preg_match('/^APP-/i',$cod) && isset($appTurnas[$cod][$turnoKey])) continue;
            ...
            $renderLinhas = function(array $lista) use ($turnoKey,$diasSemana,$diasBits,$hoje,$diasNomes,$vinculosCodigos): void {
                foreach($lista as $cod){
                    ...
                    foreach($diasSemana as $di => $data){
                        ...
                        // Verifica férias da turma → mostra 🏖️ Férias
                        ...
                        // Turma APP manual — mostra instrutor do cadastro próprio
                        if(preg_match('/^APP-/i',$cod) && isset($appTurnas[$cod][$turnoKey]) && ($bit & $appTurnas[$cod][$turnoKey]['diasSemana'])){
                            $ap = $appTurnas[$cod][$turnoKey];
                            echo "<td{$tdCls}><div class='cel-docente'>";
                            echo "<span class='cel-uc' style='color:#e65100'>APP</span>";
                            $partes = explode(' ', trim($ap['instrutor']));
                            $abrev  = $partes[0].(isset($partes[1])?' '.$partes[1]:'');
                            echo "<span class='cel-doc-nome'>".htmlspecialchars(mb_strtoupper($abrev))."</span>";
                            if($ap['nomeLab']) echo "<span class='cel-local'>".htmlspecialchars($ap['nomeLab'])."</span>";
                            echo "</div></td>";
                            continue;
                        }
