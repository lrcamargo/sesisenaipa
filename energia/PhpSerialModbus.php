...
class PhpSerialModbus
{
	...
	public function deviceInit($port='/dev/ttyUSB0', $baud=9600, $parity='none', $char=8, $sbits=1, $flow='none')
	{
	...
	public function getResponse ($raw=false,$offsetl=0,$offsetr=0)
	{
		...
		while(((microtime(true)-$startTime)<10) && ($responseString=='') ) 
		{
			// Read serial port buffer (with three seconds timeout) 
			while( ($byte = $this->serial->ReadPort()) && ((microtime(true)-$startTime)<6.0)) {
