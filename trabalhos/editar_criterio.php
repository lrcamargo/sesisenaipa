...
$stmt = sqlsrv_query($conn, "SELECT * FROM Criterios WHERE id=?", array($id));
$criterio = sqlsrv_fetch_array($stmt);

if ($_POST) {
    $descricao = $_POST['descricao'];
    $pontuacao = $_POST['pontuacao_maxima'];

    sqlsrv_query($conn, "UPDATE Criterios SET descricao=?, pontuacao_maxima=? WHERE id=?", array($descricao, $pontuacao, $id));
