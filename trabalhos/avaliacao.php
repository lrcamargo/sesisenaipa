...
$sql = "SELECT t.id, t.titulo, t.turma
        FROM Trabalhos t
        JOIN AvaliadorTrabalho at ON at.trabalho_id = t.id
        WHERE at.avaliador_id = ?";
$res = sqlsrv_query($conn, $sql, array($avaliador_id));

// Buscar avaliações já feitas
$sqlAval = "SELECT trabalho_id FROM Avaliacoes WHERE avaliador_id=?";
$resAval = sqlsrv_query($conn, $sqlAval, array($avaliador_id));
$avaliados = [];
while ($rowA = sqlsrv_fetch_array($resAval)) {
    $avaliados[] = $rowA['trabalho_id'];
}
?>
<!DOCTYPE html>
<html>
...
?>
    <div class="col-12 col-md-6 col-lg-4">
        <div class="card 4shadow-sm h-100 <?= $ja_avaliado ? 'border-success' : '' ?>" style="<?= $ja_avaliado ? 'background-color: #d4edda;' : '' ?>">
